      ...
      <div ref={rowRef} className="relative mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-3 sm:px-7">
        ...
        <div ref={rightRef} className="flex shrink-0 items-center gap-2.5 sm:gap-4">
          ...
          {user ? (
            ...
            // Always-visible text link, every screen size — no icon here
            // at all. (An earlier version added a mobile-only icon
            // alongside a width-gated text label, meant to be mutually
            // exclusive by breakpoint; in practice both ended up visible
            // together on a phone, which is exactly the redundant look
            // this replaces.) Same /login destination as the desktop
            // version always had — this isn't a second, different link.
            <Link
              to="/login"
              className={
                longScript
                  ? 'whitespace-nowrap text-[0.65rem] uppercase leading-normal tracking-wide text-moss underline decoration-gold/50'
                  : 'whitespace-nowrap text-xs uppercase leading-normal tracking-wide text-moss underline decoration-gold/50'
              }
            >
              {t('nav_sign_in')}
            </Link>
